...
import { TERMINAL_COLORS as COLORS } from '@/components/ide/terminalColors';
import { ContainerTerminalView } from '@/components/ide/ContainerTerminal';
...
/**
 * The terminal panel, in whichever environment is selected.
 *
 * The switch lives here so that every call site — the bottom panel, the mobile
 * pane, the command palette — keeps working unchanged and sees one terminal.
 * What changes underneath is the transport, and it changes completely: the
 * virtual shell is line-based against the browser's own file system, the
 * container shell is a byte stream to a PTY on a real machine. Sharing a
 * component between those would mean a component that is two components with a
...
function VirtualTerminalView({ sessionId }: { sessionId: string }) {
